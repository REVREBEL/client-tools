...
import { Users, Palette, Trash2, Plus, FileText, Sparkles, RefreshCw, Sliders, ExternalLink } from 'lucide-react';
...
const DEFAULT_STATUS_COLORS = {
  ...
  'VERIFICATION CHECKS': { bg: BRAND_COLORS.cyan, fg: BRAND_COLORS.yellow },
  'FAILED VERIFICATION': { bg: BRAND_COLORS.red, fg: BRAND_COLORS.yellow },
  'NO RESPONSE': { bg: BRAND_COLORS.orange, fg: BRAND_COLORS.purple }
};
...
function App({ data = [], updateItem, deleteItem, insertItem, moveItem, followLink }) {
  useEffect(() => { 
    ...
    @font-face { font-family: 'Khand'; font-style: normal; font-weight: 600 700; font-display: swap; src: url('https://fonts.gstatic.com/s/khand/v15/TwA0-mR39ThU2TMBMIFG.woff2') format('woff2'); } 
    ...
    @font-face { font-family: 'Roboto'; font-style: normal; font-weight: 600; font-display: swap; src: url('https://fonts.gstatic.com/s/roboto/v30/KFOlCnqEu92Fr1MmVUlfBBc-.woff2') format('woff2'); } 
  ...
  const [teamEditValues, setTeamEditValues] = useState({ fullName: '', email: '' });
  const [editingStatusLabel, setEditingStatusLabel] = useState(null);
  const [statusEditValues, setStatusEditValues] = useState({ label: '', bg: '', fg: '', index_: null });
  ...
  const COL_STATUS_BG = useMemo(() => {
    const idx = headerRow.indexOf("BACKGROUND HEX COLOR");
    return idx !== -1 ? idx : 14;
  }, [headerRow]);

  const COL_STATUS_FG = useMemo(() => {
    const idx = headerRow.indexOf("FONT HEX COLOR");
    return idx !== -1 ? idx : 15;
  }, [headerRow]);
  ...
  }, [data, COL_TEAM_FULL, COL_TEAM_EMAIL, COL_TEAM_FIRST, COL_TEAM_LAST]);

  const statusConfigs = useMemo(() => {
    const configs = [];
    const seen = new Set();
    
    // Parse existing statuses from data
    data.forEach(item => {
      if (item.index_ > 0 && item.row && item.row[COL_STATUS_NAME]) {
        const label = item.row[COL_STATUS_NAME].trim();
        if (label && !seen.has(label)) {
          seen.add(label);
          configs.push({
            index_: item.index_,
            label: label,
            bg: item.row[COL_STATUS_BG] || DEFAULT_STATUS_COLORS[label]?.bg || '#FFFFFF',
            fg: item.row[COL_STATUS_FG] || DEFAULT_STATUS_COLORS[label]?.fg || '#000000',
          });
        }
      }
    });
    
    // Add missing defaults to ensure they are available to edit
    Object.keys(DEFAULT_STATUS_COLORS).forEach(label => {
      if (!seen.has(label)) {
        configs.push({
          index_: null, // Null means it's not in the sheet yet
          label: label,
          bg: DEFAULT_STATUS_COLORS[label].bg,
          fg: DEFAULT_STATUS_COLORS[label].fg,
        });
      }
    });
  ...
  const handleStartEditTeam = (member) => {
    ...
    setTeamEditValues({ fullName: member.fullName, email: member.email });
  };
  ...
  const handleSaveTeamMember = (index_) => {
    ...
    const update = Array(COL_TEAM_EMAIL + 1).fill(undefined);
  ...
  const handleStartEditStatus = (config) => {
    setEditingStatusLabel(config.label);
    setStatusEditValues({ label: config.label, bg: config.bg, fg: config.fg, index_: config.index_ });
  };
  ...
  const handleSaveStatus = () => {
    ...
    if (statusEditValues.index_ !== null) {
      updateItem(statusEditValues.index_, update);
    } else {
      // Find an empty row or insert a new one
      const emptyRow = data.find(d => d.index_ > 0 && d.row && !d.row[COL_STATUS_NAME] && !d.row[COL_PROJECT_NAME]);
      if (emptyRow) {
  ...
  const handleDeleteTeamMember = (index_) => {
    const clearUpdate = Array(COL_TEAM_EMAIL + 1).fill(undefined);
  ...
  const handleAddStatus = (e) => {
    ...
    if (existing && existing.index_ !== null) {
  ...
  return (
    <div className="min-h-screen bg-[#EFF5F6] text-[#163666] font-roboto p-4 md:p-8 animate-in">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-[#163666]/10 pb-6 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-[#047C97]">
              ...
              <span className="font-khand font-bold text-xs uppercase tracking-normal">Configuration Hub</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {dashboardUrl && (
              <button 
                ...
                className="font-khand font-bold px-4 py-2 bg-[#00A6B6] text-white border-2 border-[#00A6B6] uppercase text-xs tracking-normal hover:bg-[#163666] hover:border-[#163666] transition-all shadow-sm flex items-center gap-2"
              >
            ...
            <div className="flex items-center gap-2 bg-[#163666]/5 px-3 py-2 border border-[#163666]/20">
              ...
              <span className="font-khand font-bold text-[10px] uppercase tracking-normal text-[#163666]/80">
                Synced: {lastSyncTime}
              </span>
            </div>
          </div>
        ...
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="font-khand font-bold text-4xl uppercase tracking-normal text-[#163666]">
            Workspace Setup
          </h1>
          <p className="font-roboto text-[#163666]/70 text-sm mt-1">
            Manage global parameters, core team members, and status color coding.
          </p>
        </div>
        ...
        <div className="bg-white p-6 border-3 border-[#163666] shadow-sm mb-8">
          <div className="flex items-center gap-2 mb-3 text-[#047C97]">
            ...
            <span className="font-khand font-bold text-xs uppercase tracking-normal">Core Identity</span>
          </div>
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="font-khand font-bold block text-[11px] text-[#163666] uppercase mb-1.5 tracking-normal">
                Project Title
              </label>
              <div className="flex items-end gap-3">
                <input 
                  ...
                  className={`font-khand font-bold flex-1 px-4 py-3 border-2 border-[#163666] text-[#163666] outline-none uppercase text-sm tracking-normal ${!isEditingProjectName ? 'bg-gray-50 opacity-80' : 'bg-white focus:ring-2 focus:ring-[#047C97]'}`}
                />
                <button 
                  onClick={() => {
                  ...
                  className="font-khand font-bold h-12 px-8 bg-[#163666] text-white uppercase text-xs tracking-normal hover:opacity-90 transition-opacity"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white p-6 md:p-8 border-3 border-[#163666] shadow-sm h-full flex flex-col">
              <div className="flex items-center gap-3 border-b-2 border-[#163666]/10 pb-4 mb-6">
                ...
                <h2 className="font-khand font-bold text-2xl uppercase tracking-normal">
              ...
              <form onSubmit={handleAddTeamMember} className="p-4 md:p-6 bg-[#EFF5F6] border border-[#163666]/30 mb-6">
                <p className="font-khand font-bold text-xs text-[#163666] uppercase tracking-normal mb-4">Add New Resource</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div>
                    <button 
                      type="submit" 
                      className="font-khand font-bold w-full h-11 text-white text-sm uppercase tracking-normal flex items-center justify-center hover:opacity-90 transition-opacity bg-[#163666]"
                    >
              ...
              <div className="grid grid-cols-1 gap-2">
                {teamMembers.map((member) => (
                  <div key={member.index_} className="flex items-center justify-between p-4 bg-white border-2 border-[#163666] hover:border-[#047C97] transition-all">
                    <div className="flex items-center gap-4">
                      <div className="font-khand font-bold w-10 h-10 bg-[#EFF5F6] flex items-center justify-center text-[#163666] text-lg uppercase border border-[#163666]">
                        {member.fullName[0]}
                      </div>
                      <div>
                        {editingTeamIndex === member.index_ ? (
                          <div className="space-y-2 py-1">
                            <input 
                              className="font-khand font-bold block w-full px-2 py-1 text-xs border border-[#163666] uppercase tracking-normal" 
                          ...
                          <>
                            <p className="font-khand font-bold text-[#163666] text-sm uppercase tracking-normal leading-none">{member.fullName}</p>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => editingTeamIndex === member.index_ ? handleSaveTeamMember(member.index_) : handleStartEditTeam(member)} className="font-khand font-bold px-3 py-1.5 text-[10px] uppercase border border-[#163666]/20 hover:bg-[#163666] hover:text-white transition-all tracking-normal">
                ...
                {teamMembers.length === 0 && (
                  <div className="font-khand font-bold py-12 text-center border-2 border-dashed border-[#163666]/20 text-[#163666]/40 uppercase text-xs tracking-normal">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 md:p-8 border-3 border-[#163666] shadow-sm h-full flex flex-col">
              ...
              {/* Status List */}
              <div className="space-y-2 mb-8 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                {statusConfigs.map((config) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-4 h-4 border border-black/20" style={{ backgroundColor: config.bg }} />
                      {editingStatusLabel === config.label ? (
                        <input className="font-khand font-bold bg-transparent border-b border-current outline-none uppercase tracking-normal w-24" value={statusEditValues.label} onChange={e => setStatusEditValues({...statusEditValues, label: e.target.value})} />
                      ) : (
                        <span className="uppercase tracking-normal truncate">{config.label}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="font-roboto text-[10px] shrink-0 opacity-90">
                        <span className="px-1.5 py-0.5 bg-black/10 border border-black/10">{config.bg}</span>
                      </div>
                      <button onClick={() => editingStatusLabel === config.label ? handleSaveStatus() : handleStartEditStatus(config)} className="font-khand font-bold px-2 py-1 bg-white/20 hover:bg-white/40 border border-current text-[9px] uppercase tracking-normal">
                        {editingStatusLabel === config.label ? 'Save' : 'Edit'}
                      </button>
                    </div>
                  </div>
                ))}
                {statusConfigs.length === 0 && (
                  <div className="font-khand font-bold py-8 text-center border-2 border-dashed border-[#163666]/20 text-[#163666]/40 uppercase text-xs tracking-normal">
              ...
              <form onSubmit={handleAddStatus} className="p-4 bg-[#EFF5F6] border border-[#163666]/30 space-y-4">
                <div className="flex items-center gap-2 text-[#163666]/80 border-b border-[#163666]/15 pb-2">
                  ...
                  <p className="font-khand font-bold text-xs text-[#163666] uppercase tracking-normal">Status Color Config</p>
                </div>

                <div>
                  <label className="font-khand font-bold block text-[10px] text-[#163666] mb-1 uppercase tracking-normal">Status Name</label>
                  <input 
                    type="text" 
                    value={newColorLabel}
                    ...
                    required 
                    className="font-khand font-bold w-full px-3 py-2 border-2 border-[#163666] text-xs outline-none bg-white uppercase tracking-normal text-[#163666] focus:border-[#047C97]" 
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="font-khand font-bold block text-[10px] text-[#163666] mb-1 uppercase tracking-normal">BG HEX</label>
                    <div className="flex items-center gap-2">
                      <input 
                        type="text" 
                        value={newColorBg}
                  ...
                  <div>
                    <label className="font-khand font-bold block text-[10px] text-[#163666] mb-1 uppercase tracking-normal">TEXT HEX</label>
                    <div className="flex items-center gap-2">
                      <input 
                        type="text" 
                        value={newColorFg}
                ...
                <button 
                  type="submit" 
                  className="font-khand font-bold w-full py-2.5 text-white hover:opacity-90 transition-opacity text-xs uppercase tracking-normal bg-[#163666]"
                >
                  Save Status Color
                </button>
              </form>
