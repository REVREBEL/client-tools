...
export default function TopNavigation({
  ...
  const getActiveClass = (isActive: boolean) =>
    isActive ? 'bg-brand-primary text-brand-powder' : 'text-slate-500 hover:text-brand-primary';

  return (
    <header className="mb-8">
      {/* Navigation Bar & Header Row */}
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center mb-6">
        ...
        <div className="flex items-center h-8.75">
          <svg
            ...
            viewBox="0 0 1180 174.8"
            className="h-full w-auto"
          >
            <defs>
              <style>{".b{fill:#1c3766;}"}</style>
            </defs>
        ...
        <div className="flex items-center gap-1 bg-slate-200/50 p-1 rounded-none w-fit border-2 border-brand-primary">
          <button
            onClick={() => setActiveTab('dashboard')}
            className={"px-5 pt-3 pb-2.5 rounded-none text-base font-bold tracking-wide transition-all font-khand uppercase " + getActiveClass(activeTab === 'dashboard')}
          >
            STRATEGY ITEMS
          </button>
          <button
            onClick={() => setActiveTab('tracking')}
            className={"px-5 pt-3 pb-2.5 rounded-none text-base font-bold tracking-wide transition-all font-khand uppercase " + getActiveClass(activeTab === 'tracking')}
          >
            Tracking Dashboard
          </button>
          <button
            onClick={() => setActiveTab('sequencer')}
            className={"px-5 pt-3 pb-2.5 rounded-none text-base font-bold tracking-wide transition-all font-khand uppercase " + getActiveClass(activeTab === 'sequencer')}
          >
            Sequence Arranger
          </button>
          <button
            onClick={() => targetHyperlink && followLink && followLink(targetHyperlink)}
            className="px-5 pt-3 pb-2.5 rounded-none text-sm font-bold tracking-wide transition-all font-khand text-slate-500 hover:text-brand-primary uppercase"
          >
            Settings
          </button>
        </div>
      ...
      {/* Project Title & New Task Trigger Action */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight font-khand uppercase pt-0.5 text-brand-primary">
            {displayProjectName.toUpperCase()}
          </h1>
        </div>
        <button
          onClick={() => setIsAddingTask(true)}
          className="flex items-center gap-2 px-5 pt-3.5 pb-3 rounded-none font-bold shadow-lg transition-all hover:shadow-xl hover:-translate-y-0.5 font-khand tracking-wide uppercase bg-brand-primary text-brand-powder"
        >
          <Plus className="w-5 h-5" />
          <span className="pt-0.5">New Task</span>
        </button>
      </div>
    </header>
  );
